...
<Tabs>
  ...
  <TabContent title="Docker">
    `npm run export` is a wrapper around this Docker command. If Node.js is not installed locally, run it directly:
...
The production build always runs inside the Node.js container, so both commands produce identical output. If the Docker stack was not already running, `npm run export` automatically stops it again when the build finishes.
